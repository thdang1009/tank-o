...
import { TankClassType } from '../constants/ClassSelection';
import { GameMode } from '../constants/GameModes';
import { MapType } from '../constants/MapEnum';
import { SocketService, SocketEvents, Lobby, Player } from '../services/SocketService';

// Tank classes and their descriptions
const TankClasses: Record<TankClassType, { name: string; description: string; asset: string; specialty: string }> = {
    [TankClassType.BALANCED]: {
        name: 'Balanced',
        description: 'Well-rounded tank with no weaknesses',
        asset: AssetsEnum.TANK_BALANCED,
        specialty: 'Balanced stats'
    },
    [TankClassType.HEAVY]: {
        name: 'Heavy',
        description: 'Slow but powerful tank with high health',
        asset: AssetsEnum.TANK_HEAVY,
        specialty: 'High health'
    },
    [TankClassType.SCOUT]: {
        name: 'Scout',
        description: 'Fast and agile tank with low health',
        asset: AssetsEnum.TANK_SCOUT,
        specialty: 'High speed'
    },
    [TankClassType.SNIPER]: {
        name: 'Sniper',
        description: 'Long-range tank with high damage',
        asset: AssetsEnum.TANK_SNIPER,
...
export class LobbyScene extends Scene {
    ...
    private selectedTankClass: TankClassType = TankClassType.BALANCED;
    ...
    private createNewLobby() {
        ...
        const initialMapType = MapType.MIXED;
    ...
    private joinExistingLobby() {
        const lobbyId = this.scene.settings.data?.lobbyId;
    ...
    private setupSocketEventListeners() {
        ...
        this.socketService.on(SocketEvents.PLAYER_JOINED, (player: Player) => {
    ...
    private updatePlayerList() {
        ...
        players.forEach((player, index) => {
            ...
            if (player.tankClass) {
                ...
                if (tankClassInfo) {
                    const tankIcon = this.add.image(-80, 0, tankClassInfo.asset)
                        .setScale(0.3);
    ...
    private createTankClassSection() {
        ...
        this.classButtons = classTypes.map((classType, index) => {
            ...
            // Tank icon
            const tankIcon = this.add.image(0, -10, tankInfo.asset)
                .setScale(0.4);
    ...
    private selectTankClass(classType: TankClassType) {
        ...
        this.classButtons.forEach(button => {
            const tankName = (button.getAt(2) as GameObjects.Text).text;
            const tankClass = Object.values(TankClassType).find(type => 
                TankClasses[type].name === tankName);
            const rect = button.getAt(0) as GameObjects.Rectangle;
